...
export function Appointment({ withHeader = true }: { withHeader?: boolean }) {
      ...
      <div className="container-luxe grid lg:grid-cols-2 gap-16 items-start">
        <div className="lg:sticky lg:top-32">
          {withHeader && (
            <>
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-[11px] uppercase tracking-[0.4em] text-rose">Rendez-vous</span>
              </div>
              <h2 className="text-4xl md:text-5xl lg:text-6xl leading-[1.1]">
                Offrez-vous une <em className="italic text-gradient-rose">parenthèse</em> d'exception.
              </h2>
            </>
          )}
          ...
          <div className="mt-10 space-y-5">
            {[
              ...
              ["Email", "contact@nassimbentayeb.ma"],
              ["Adresse", "Avenue Hassan II, Quartier Al Matar, Nador 62000, Maroc"],
              ["Horaires", "Lun – Sam · 9h30 – 20h00"],
            ].map(([k, v]) => (
              <div key={k} className="flex items-baseline gap-6 border-b border-border/60 pb-4">
                <span className="text-[10px] uppercase tracking-[0.3em] text-rose w-20 shrink-0">{k}</span>
          ...
          <a
            ...
            target="_blank"
            rel="noopener"
            className="mt-10 inline-flex items-center gap-3 rounded-full bg-[#25D366] px-7 py-3 text-xs uppercase tracking-[0.25em] text-white hover:shadow-glow transition-all duration-500"
          >
        ...
        >
          <div className="grid sm:grid-cols-2 gap-5">
            ...
            <Field label="Téléphone" name="tel" type="tel" required placeholder="+212 6 ..." />
          </div>
          <Field label="Email" name="email" type="email" required />
          <div>
            <label className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground">Soin souhaité</label>
            <select required className="mt-2 w-full border-b border-border bg-transparent py-3 text-foreground focus:border-rose focus:outline-none transition">
          ...
          <div>
            ...
            <textarea rows={4} className="mt-2 w-full border-b border-border bg-transparent py-3 text-foreground focus:border-rose focus:outline-none transition resize-none" placeholder="Partagez-nous vos attentes…" />
          </div>
          <button type="submit" className="w-full inline-flex items-center justify-center gap-3 rounded-full bg-gradient-rose px-8 py-4 text-xs uppercase tracking-[0.25em] text-white hover:shadow-glow transition-all duration-500 shadow-soft">
            {sent ? "✓ Demande envoyée" : "Réserver maintenant"}
          </button>
          <p className="text-[10px] text-muted-foreground text-center uppercase tracking-[0.2em]">
            Réponse sous 24h · Données 100% confidentielles
          </p>
        </form>
      </div>
    </section>
  ...
  return (
    <div>
      ...
      <input
        ...
        className="mt-2 w-full border-b border-border bg-transparent py-3 text-foreground placeholder:text-muted-foreground/60 focus:border-rose focus:outline-none transition"
      />
    </div>
  );
}
